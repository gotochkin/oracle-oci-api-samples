...
//
/// Update an instance using PU and http package
///
...
func OciPut(endPoint string, instanceId string, profile string, putBody string) (status string) {
	fmt.Println("Start")
	// Create URL
	restApi := "/20160918/instances"

	url := endPoint + restApi + "/" + instanceId
	fmt.Println(url)

	bodyReader, err := os.Open(putBody)
	if err != nil {
		log.Fatal(err)
	}
